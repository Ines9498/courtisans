document.addEventListener("DOMContentLoaded", function () {
  ...
  function creerDomaineAdversaire(numero) {
    const familles = ["papillon", "crapaud", "rossignol", "espion", "lapin", "cerf", "carpe"];
  
    const wrapper = document.createElement("div");
    wrapper.className = "wrapper-adversaire";
  
    const titre = document.createElement("h4");
    titre.textContent = `Adversaire ${numero}`;
  
    const conteneur = document.createElement("div"); // ✅ Créé avant qu’on l’utilise
    ...
    familles.forEach(famille => {
      ...
      zone.addEventListener("click", handleZoneClick); // 👈 ici !
      conteneur.appendChild(zone);
    });
    
  
    wrapper.appendChild(titre);       // ✅ On met d’abord le titre dans le wrapper
    wrapper.appendChild(conteneur);   // ✅ Puis les zones
    document.getElementById("domaine-adversaires").appendChild(wrapper); // ✅ On ajoute le tout au DOM
  }
